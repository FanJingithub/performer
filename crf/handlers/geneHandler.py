...
class geneHandler(tornado.web.RequestHandler):
    ...
    def get(self):
        ...
        self.cur.execute('''SELECT gene FROM data_status WHERE patient_id='{0}' '''.format(self.patient_id))
        for row in self.cur:
            page_status = row[0]
        ...
        if (exist==0):
            ...
            sql = "UPDATE data_status SET gene='" + page_status + "' WHERE patient_id='" + self.patient_id + "'"
        ...
        if page_index>0:
            page_previous = "/gene?patient_id="+self.patient_id+"&page_index="+str(page_index-1)
        else:
            page_previous = "/base?patient_id="+self.patient_id
        if (submitted=="1" and edit=="0"):
            self.render("../html/read_gene_page.html", side_menu=side_menu, page_index=page_index, page_previous=page_previous, patient_id=self.patient_id,  gene_result=res[1])
        else:
            self.render("../html/edit_gene_page.html", side_menu=side_menu, page_index=page_index, page_previous=page_previous, patient_id=self.patient_id,  gene_result=res[1])
    ...
    def post(self):
        ...
        try:
            ...
            page_index = "0"
        page_index = int(page_index)
        gene_result = self.get_body_argument("gene_result") 
        ...
        self.cur = conn.cursor()
        sql = '''REPLACE INTO gene_{0} (patient_id, gene_result) VALUES ('{1}', '{2}') '''.format(page_index, self.patient_id, gene_result)
        self.cur.execute(sql)

        sql = "SELECT gene FROM data_status WHERE patient_id='" + self.patient_id + "'"
        self.cur.execute(sql)
        page_status = "0"
        for row in self.cur:
            page_status = row[0]
        ...
        sql = "UPDATE data_status SET gene=" + page_status_new + " WHERE patient_id='" + self.patient_id + "'"
