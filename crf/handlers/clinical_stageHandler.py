...
class clinical_stageHandler(tornado.web.RequestHandler):

    def get(self):
        print('----------------------------Get clinical_stage--------------------------')
        ...
        self.cur.execute('''SELECT clinical_stage FROM data_status WHERE patient_id='{0}' '''.format(self.patient_id))
        for row in self.cur:
            page_status = row[0]
        ...
        if (exist==0):
            ...
            sql = "UPDATE data_status SET clinical_stage='" + page_status + "' WHERE patient_id='" + self.patient_id + "'"
        ...
        if page_index>0:
            page_previous = "/clinical_stage?patient_id="+self.patient_id+"&page_index="+str(page_index-1)
        else:
            page_previous = "/base?patient_id="+self.patient_id
        if (submitted=="1" and edit=="0"):
            self.render("../html/read_clinical_stage_page.html", side_menu=side_menu, page_index=page_index, page_previous=page_previous, patient_id=self.patient_id,  c_t_stage=res[1], c_n_stage=res[2], c_m_stage=res[3], final_stage=res[4])
        else:
            self.render("../html/edit_clinical_stage_page.html", side_menu=side_menu, page_index=page_index, page_previous=page_previous, patient_id=self.patient_id,  c_t_stage=res[1], c_n_stage=res[2], c_m_stage=res[3], final_stage=res[4])
    ...
    def post(self):
        ...
        try:
            ...
            page_index = "0"
        page_index = int(page_index)
        c_t_stage = self.get_body_argument("c_t_stage") 
        c_n_stage = self.get_body_argument("c_n_stage") 
        c_m_stage = self.get_body_argument("c_m_stage") 
        final_stage = self.get_body_argument("final_stage") 
        ...
        self.cur = conn.cursor()
        sql = '''REPLACE INTO clinical_stage_{0} (patient_id, c_t_stage, c_n_stage, c_m_stage, final_stage) VALUES ('{1}', '{2}', '{3}', '{4}', '{5}') '''.format(page_index, self.patient_id, c_t_stage,c_n_stage,c_m_stage,final_stage)
        self.cur.execute(sql)

        sql = "SELECT clinical_stage FROM data_status WHERE patient_id='" + self.patient_id + "'"
        self.cur.execute(sql)
        page_status = "0"
        for row in self.cur:
            page_status = row[0]
        ...
        sql = "UPDATE data_status SET clinical_stage=" + page_status_new + " WHERE patient_id='" + self.patient_id + "'"
