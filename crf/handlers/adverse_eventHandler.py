...
class adverse_eventHandler(tornado.web.RequestHandler):

    def get(self):
        print('----------------------------Get adverse_event--------------------------')
        ...
        self.cur.execute('''SELECT adverse_event FROM data_status WHERE patient_id='{0}' '''.format(self.patient_id))
        for row in self.cur:
            page_status = row[0]
        ...
        if (exist==0):
            ...
            sql = "UPDATE data_status SET adverse_event='" + page_status + "' WHERE patient_id='" + self.patient_id + "'"
        ...
        if page_index>0:
            page_previous = "/adverse_event?patient_id="+self.patient_id+"&page_index="+str(page_index-1)
        else:
            page_previous = "/base?patient_id="+self.patient_id
        if (submitted=="1" and edit=="0"):
            self.render("../html/read_adverse_event_page.html", side_menu=side_menu, page_index=page_index, page_previous=page_previous, patient_id=self.patient_id,  adverse_type=res[1], start_time=res[2], end_time=res[3], nci_ctc=res[4], disposal=res[5], disposal_other=res[6], outcome=res[7], drug_relation=res[8])
        else:
            self.render("../html/edit_adverse_event_page.html", side_menu=side_menu, page_index=page_index, page_previous=page_previous, patient_id=self.patient_id,  adverse_type=res[1], start_time=res[2], end_time=res[3], nci_ctc=res[4], disposal=res[5], disposal_other=res[6], outcome=res[7], drug_relation=res[8])
    ...
    def post(self):
        ...
        try:
            ...
            page_index = "0"
        page_index = int(page_index)
        adverse_type = self.get_body_argument("adverse_type") 
        start_time = self.get_body_argument("start_time") 
        end_time = self.get_body_argument("end_time") 
        nci_ctc = self.get_body_argument("nci_ctc") 
        disposal = self.get_body_argument("disposal") 
        disposal_other = self.get_body_argument("disposal_other") 
        outcome = self.get_body_argument("outcome") 
        drug_relation = self.get_body_argument("drug_relation") 
        ...
        self.cur = conn.cursor()
        sql = '''REPLACE INTO adverse_event_{0} (patient_id, adverse_type, start_time, end_time, nci_ctc, disposal, disposal_other, outcome, drug_relation) VALUES ('{1}', '{2}', '{3}', '{4}', '{5}', '{6}', '{7}', '{8}', '{9}') '''.format(page_index, self.patient_id, adverse_type,start_time,end_time,nci_ctc,disposal,disposal_other,outcome,drug_relation)
        self.cur.execute(sql)

        sql = "SELECT adverse_event FROM data_status WHERE patient_id='" + self.patient_id + "'"
        self.cur.execute(sql)
        page_status = "0"
        for row in self.cur:
            page_status = row[0]
        ...
        sql = "UPDATE data_status SET adverse_event=" + page_status_new + " WHERE patient_id='" + self.patient_id + "'"
