...
class colonoscopy_pathologyHandler(tornado.web.RequestHandler):

    def get(self):
        print('----------------------------Get colonoscopy_pathology--------------------------')
        ...
        self.cur.execute('''SELECT colonoscopy_pathology FROM data_status WHERE patient_id='{0}' '''.format(self.patient_id))
        for row in self.cur:
            page_status = row[0]
        ...
        if (exist==0):
            ...
            sql = "UPDATE data_status SET colonoscopy_pathology='" + page_status + "' WHERE patient_id='" + self.patient_id + "'"
        ...
        if page_index>0:
            page_previous = "/colonoscopy_pathology?patient_id="+self.patient_id+"&page_index="+str(page_index-1)
        else:
            page_previous = "/base?patient_id="+self.patient_id
        if (submitted=="1" and edit=="0"):
            self.render("../html/read_colonoscopy_pathology_page.html", side_menu=side_menu, page_index=page_index, page_previous=page_previous, patient_id=self.patient_id,  first_path_diagnose_date=res[1], pathology_type=res[2], pathology_type_other=res[3], degree_of_differentiation=res[4])
        else:
            self.render("../html/edit_colonoscopy_pathology_page.html", side_menu=side_menu, page_index=page_index, page_previous=page_previous, patient_id=self.patient_id,  first_path_diagnose_date=res[1], pathology_type=res[2], pathology_type_other=res[3], degree_of_differentiation=res[4])
    ...
    def post(self):
        ...
        try:
            ...
            page_index = "0"
        page_index = int(page_index)
        first_path_diagnose_date = self.get_body_argument("first_path_diagnose_date") 
        pathology_type = self.get_body_argument("pathology_type") 
        pathology_type_other = self.get_body_argument("pathology_type_other") 
        degree_of_differentiation = self.get_body_argument("degree_of_differentiation") 
        ...
        self.cur = conn.cursor()
        sql = '''REPLACE INTO colonoscopy_pathology_{0} (patient_id, first_path_diagnose_date, pathology_type, pathology_type_other, degree_of_differentiation) VALUES ('{1}', '{2}', '{3}', '{4}', '{5}') '''.format(page_index, self.patient_id, first_path_diagnose_date,pathology_type,pathology_type_other,degree_of_differentiation)
        self.cur.execute(sql)

        sql = "SELECT colonoscopy_pathology FROM data_status WHERE patient_id='" + self.patient_id + "'"
        self.cur.execute(sql)
        page_status = "0"
        for row in self.cur:
            page_status = row[0]
        ...
        sql = "UPDATE data_status SET colonoscopy_pathology=" + page_status_new + " WHERE patient_id='" + self.patient_id + "'"
