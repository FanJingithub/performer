...
class concomitant_medicationHandler(tornado.web.RequestHandler):

    def get(self):
        print('----------------------------Get concomitant_medication--------------------------')
        ...
        self.cur.execute('''SELECT concomitant_medication FROM data_status WHERE patient_id='{0}' '''.format(self.patient_id))
        for row in self.cur:
            page_status = row[0]
        ...
        if (exist==0):
            ...
            sql = "UPDATE data_status SET concomitant_medication='" + page_status + "' WHERE patient_id='" + self.patient_id + "'"
        ...
        if page_index>0:
            page_previous = "/concomitant_medication?patient_id="+self.patient_id+"&page_index="+str(page_index-1)
        else:
            page_previous = "/base?patient_id="+self.patient_id
        if (submitted=="1" and edit=="0"):
            self.render("../html/read_concomitant_medication_page.html", side_menu=side_menu, page_index=page_index, page_previous=page_previous, patient_id=self.patient_id,  drug=res[1], start_time=res[2], end_time=res[3], dose=res[4], method=res[5], reason=res[6])
        else:
            self.render("../html/edit_concomitant_medication_page.html", side_menu=side_menu, page_index=page_index, page_previous=page_previous, patient_id=self.patient_id,  drug=res[1], start_time=res[2], end_time=res[3], dose=res[4], method=res[5], reason=res[6])
    ...
    def post(self):
        ...
        self.cur = conn.cursor()
        sql = '''REPLACE INTO concomitant_medication_{0} (patient_id, drug, start_time, end_time, dose, method, reason) VALUES ('{1}', '{2}', '{3}', '{4}', '{5}', '{6}', '{7}') '''.format(page_index, self.patient_id, drug,start_time,end_time,dose,method,reason)
        self.cur.execute(sql)

        sql = "SELECT concomitant_medication FROM data_status WHERE patient_id='" + self.patient_id + "'"
        self.cur.execute(sql)
        page_status = "0"
        for row in self.cur:
            page_status = row[0]
        ...
        sql = "UPDATE data_status SET concomitant_medication=" + page_status_new + " WHERE patient_id='" + self.patient_id + "'"
