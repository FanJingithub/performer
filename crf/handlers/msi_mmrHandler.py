...
class msi_mmrHandler(tornado.web.RequestHandler):
    ...
    def get(self):
        ...
        self.cur.execute('''SELECT msi_mmr FROM data_status WHERE patient_id='{0}' '''.format(self.patient_id))
        for row in self.cur:
            page_status = row[0]
        ...
        if (exist==0):
            ...
            sql = "UPDATE data_status SET msi_mmr='" + page_status + "' WHERE patient_id='" + self.patient_id + "'"
        ...
        if page_index>0:
            page_previous = "/msi_mmr?patient_id="+self.patient_id+"&page_index="+str(page_index-1)
        else:
            page_previous = "/base?patient_id="+self.patient_id
        if (submitted=="1" and edit=="0"):
            self.render("../html/read_msi_mmr_page.html", side_menu=side_menu, page_index=page_index, page_previous=page_previous, patient_id=self.patient_id,  msi_result=res[1], mmr_result=res[2])
        else:
            self.render("../html/edit_msi_mmr_page.html", side_menu=side_menu, page_index=page_index, page_previous=page_previous, patient_id=self.patient_id,  msi_result=res[1], mmr_result=res[2])
    ...
    def post(self):
        ...
        try:
            ...
            page_index = "0"
        page_index = int(page_index)
        msi_result = self.get_body_argument("msi_result") 
        mmr_result = self.get_body_argument("mmr_result") 
        ...
        self.cur = conn.cursor()
        sql = '''REPLACE INTO msi_mmr_{0} (patient_id, msi_result, mmr_result) VALUES ('{1}', '{2}', '{3}') '''.format(page_index, self.patient_id, msi_result,mmr_result)
        self.cur.execute(sql)

        sql = "SELECT msi_mmr FROM data_status WHERE patient_id='" + self.patient_id + "'"
        self.cur.execute(sql)
        page_status = "0"
        for row in self.cur:
            page_status = row[0]
        ...
        sql = "UPDATE data_status SET msi_mmr=" + page_status_new + " WHERE patient_id='" + self.patient_id + "'"
