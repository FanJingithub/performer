...
class treatmentHandler(tornado.web.RequestHandler):
    ...
    def get(self):
        ...
        self.cur.execute('''SELECT treatment FROM data_status WHERE patient_id='{0}' '''.format(self.patient_id))
        for row in self.cur:
            page_status = row[0]
        ...
        if (exist==0):
            ...
            sql = "UPDATE data_status SET treatment='" + page_status + "' WHERE patient_id='" + self.patient_id + "'"
        ...
        if page_index>0:
            page_previous = "/treatment?patient_id="+self.patient_id+"&page_index="+str(page_index-1)
        else:
            page_previous = "/base?patient_id="+self.patient_id
        if (submitted=="1" and edit=="0"):
            self.render("../html/read_treatment_page.html", side_menu=side_menu, page_index=page_index, page_previous=page_previous, patient_id=self.patient_id,  group=res[1], treat_start_time=res[2], treat_end_time=res[3], treat_count=res[4], stop_reason=res[5], progress_time=res[6], dead_time=res[7])
        else:
            self.render("../html/edit_treatment_page.html", side_menu=side_menu, page_index=page_index, page_previous=page_previous, patient_id=self.patient_id,  group=res[1], treat_start_time=res[2], treat_end_time=res[3], treat_count=res[4], stop_reason=res[5], progress_time=res[6], dead_time=res[7])
    ...
    def post(self):
        ...
        treat_start_time = self.get_body_argument("treat_start_time") 
        treat_end_time = self.get_body_argument("treat_end_time") 
        treat_count = self.get_body_argument("treat_count") 
        stop_reason = self.get_body_argument("stop_reason") 
        progress_time = self.get_body_argument("progress_time") 
        dead_time = self.get_body_argument("dead_time") 
        ...
        self.cur = conn.cursor()
        sql = '''REPLACE INTO treatment_{0} (patient_id, group, treat_start_time, treat_end_time, treat_count, stop_reason, progress_time, dead_time) VALUES ('{1}', '{2}', '{3}', '{4}', '{5}', '{6}', '{7}', '{8}') '''.format(page_index, self.patient_id, group,treat_start_time,treat_end_time,treat_count,stop_reason,progress_time,dead_time)
        self.cur.execute(sql)

        sql = "SELECT treatment FROM data_status WHERE patient_id='" + self.patient_id + "'"
        self.cur.execute(sql)
        page_status = "0"
        for row in self.cur:
            page_status = row[0]
        ...
        sql = "UPDATE data_status SET treatment=" + page_status_new + " WHERE patient_id='" + self.patient_id + "'"
