...
class radiotherapyHandler(tornado.web.RequestHandler):
    ...
    def get(self):
        ...
        if (exist==0):
            ...
            sql = "UPDATE data_status SET radiotherapy='" + page_status + "' WHERE patient_id='" + self.patient_id + "'"
        ...
        if page_index>0:
            page_previous = "/radiotherapy?patient_id="+self.patient_id+"&page_index="+str(page_index-1)
        else:
            page_previous = "/base?patient_id="+self.patient_id
        if (submitted=="1" and edit=="0"):
            self.render("../html/read_radiotherapy_page.html", side_menu=side_menu, page_index=page_index, page_previous=page_previous, patient_id=self.patient_id,  radio_count=res[1], radio_start=res[2], radio_end=res[3])
        else:
            self.render("../html/edit_radiotherapy_page.html", side_menu=side_menu, page_index=page_index, page_previous=page_previous, patient_id=self.patient_id,  radio_count=res[1], radio_start=res[2], radio_end=res[3])
    ...
    def post(self):
        ...
        self.cur = conn.cursor()
        sql = '''REPLACE INTO radiotherapy_{0} (patient_id, radio_count, radio_start, radio_end) VALUES ('{1}', '{2}', '{3}', '{4}') '''.format(page_index, self.patient_id, radio_count,radio_start,radio_end)
        self.cur.execute(sql)

        sql = "SELECT radiotherapy FROM data_status WHERE patient_id='" + self.patient_id + "'"
        self.cur.execute(sql)
        page_status = "0"
        for row in self.cur:
            page_status = row[0]
        ...
        sql = "UPDATE data_status SET radiotherapy=" + page_status_new + " WHERE patient_id='" + self.patient_id + "'"
