...
class labHandler(tornado.web.RequestHandler):
    ...
    def get(self):
        ...
        if (submitted=="1"):
            ...
            self.cur.execute('''SELECT patient_id,CEA,CA199 FROM lab_{0} WHERE patient_id='{1}' '''.format(page_index,self.patient_id))
            for row in self.cur:
                res = row
        ...
        if (submitted=="1" and edit=="0"):
            self.render("../html/read_lab_page.html", side_menu=side_menu, page_index=page_index, page_previous=page_previous, patient_id=self.patient_id,  CEA=res[1], CA199=res[2])
        else:
            self.render("../html/edit_lab_page.html", side_menu=side_menu, page_index=page_index, page_previous=page_previous, patient_id=self.patient_id,  CEA=res[1], CA199=res[2])
    ...
    def post(self):
        ...
        self.cur = conn.cursor()
        sql = '''REPLACE INTO lab_{0} (patient_id, CEA, CA199) VALUES ('{1}', '{2}', '{3}') '''.format(page_index, self.patient_id, CEA,CA199)
        self.cur.execute(sql)
