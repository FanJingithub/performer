...
class follow_upHandler(tornado.web.RequestHandler):
    ...
    def get(self):
        ...
        if (submitted=="1" and edit=="0"):
            self.render("../html/read_follow_up_page.html", side_menu=side_menu, page_index=page_index, page_previous=page_previous, patient_id=self.patient_id,  recurrance=res[1])
        else:
            self.render("../html/edit_follow_up_page.html", side_menu=side_menu, page_index=page_index, page_previous=page_previous, patient_id=self.patient_id,  recurrance=res[1])
    ...
    def post(self):
        ...
        self.cur = conn.cursor()
        sql = '''REPLACE INTO follow_up_{0} (patient_id, recurrance) VALUES ('{1}', '{2}') '''.format(page_index, self.patient_id, recurrance)
        self.cur.execute(sql)
